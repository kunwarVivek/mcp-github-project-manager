...
describe("DuplicateDetectionService", () => {
  ...
  const mockExistingIssues = [
    {
      ...
      body: "The login button fails to respond",
      labels: [],
      state: "open" as const,
    ...
    },
    {
      ...
      body: "Theme preference resets after restart",
      labels: [],
      state: "open" as const,
  ...
  beforeEach(() => {
    ...
    mockEmbedMany.mockResolvedValue({
      embeddings: mockExistingIssues.map(() => [0.1, 0.2, 0.3]),
    } as any);
  ...
  describe("Embedding Path", () => {
    ...
    it("should tier results correctly", async () => {
      ...
      mockCosineSimilarity
        ...
        .mockReturnValueOnce(0.8) // Medium
        .mockReturnValueOnce(0.6) // Low
        .mockReturnValueOnce(0.4); // Not included
      ...
      expect(
        result.highConfidence.length + result.mediumConfidence.length + result.lowConfidence.length
      ).toBeLessThanOrEqual(mockExistingIssues.length);
    });
