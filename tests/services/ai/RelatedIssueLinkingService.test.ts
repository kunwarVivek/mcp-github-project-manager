...
describe("RelatedIssueLinkingService", () => {
  ...
  const mockRepositoryIssues = [
    ...
    },
    {
      ...
      number: 2,
      title: "Dashboard UI",
      body: "Create dashboard layout with charts",
      labels: ["frontend", "ui"],
      state: "open" as const,
    ...
    },
    {
      ...
      body: "Implement rate limits. Requires auth to be done first.",
      labels: ["backend", "api"],
      state: "open" as const,
    ...
    },
    {
      ...
      body: "Display user profile with settings. Blocked by auth implementation.",
    ...
    },
    {
      ...
      body: "Implement password reset flow for auth",
      labels: ["auth", "backend"],
  ...
  beforeEach(() => {
    ...
    mockEmbedMany.mockResolvedValue({
      embeddings: mockRepositoryIssues.map(() => [0.1, 0.2, 0.3]),
    } as any);
  ...
  describe("Semantic Relations", () => {
    it("should find semantically similar issues via embeddings", async () => {
      ...
      const semanticRelations = result.relationships.filter(
        (r) => r.relationshipType === "semantic"
      );
  ...
  describe("Component Grouping", () => {
    it("should group issues by shared labels", async () => {
      ...
      const componentRelations = result.relationships.filter(
        (r) => r.relationshipType === "component"
      );
