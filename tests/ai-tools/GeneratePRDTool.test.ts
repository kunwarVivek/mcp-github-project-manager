...
vi.mock("../../src/services/PRDGenerationService", () => ({
  PRDGenerationService: vi.fn().mockImplementation(function () {
    return {
...
describe("GeneratePRDTool", () => {
  ...
  beforeEach(() => {
    ...
    (PRDGenerationService as Mock).mockImplementation(function () {
      return mockPRDService;
    });
  });
  ...
  describe("executeGeneratePRD", () => {
    it("should generate PRD successfully with complete validation", async () => {
      ...
      const args = {
        projectIdea:
          "A modern task management application with team collaboration features and real-time updates",
    ...
    it("should handle PRD generation with market research", async () => {
      ...
      const args = {
        projectIdea:
          "AI-powered fitness tracking app with personalized coaching and social features",
    ...
    it("should propagate genuine (non-AI-unavailable) errors as a rejection so the MCP dispatcher can set isError", async () => {
      mockPRDService.generatePRDFromIdea.mockRejectedValue(
    ...
    it("should include comprehensive PRD sections in output", async () => {
      const mockComprehensivePRD = {
        ...
        targetUsers: [
          {
            name: "Executive",
            description: "C-level decision makers",
