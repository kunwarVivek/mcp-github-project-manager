...
vi.mock("../../src/services/ProjectManagementService", () => ({
  ProjectManagementService: vi.fn().mockImplementation(function () {
    return {
      ...
      createIssueComment: vi
        .fn()
        .mockResolvedValue({ id: 1, body: "", user: "", createdAt: "", updatedAt: "" }),
    };
  }),
}));
...
vi.mock("../../src/services/IssueEnrichmentService", () => ({
  IssueEnrichmentService: vi.fn().mockImplementation(function () {
    return {
      enrichIssue: vi.fn(),
...
describe("IssueTriagingService", () => {
  ...
  beforeEach(() => {
    ...
    (IssueEnrichmentService as Mock).mockImplementation(() => mockEnrichmentService);
  ...
  describe("triageIssue", () => {
    ...
    it("should identify non-actionable issues", async () => {
      ...
      expect(result.actions).toContainEqual(expect.objectContaining({ type: "close" }));
    });
    ...
    it("should triage documentation issues with low priority", async () => {
      ...
      expect(result.actions).toContainEqual(expect.objectContaining({ value: "good-first-issue" }));
    });
  });
