...
describe("RequirementsTraceabilityService", () => {
  ...
  describe("generateUseCasesFromFeature", () => {
    it("should generate use cases from feature user stories", () => {
      ...
      const useCases = service.generateUseCasesFromFeature(
        mockFeature as any,
      ...
      expect(useCases[0].postconditions).toEqual(
        expect.arrayContaining([expect.stringContaining("track work progress")])
      );
  ...
  describe("relationship detection", () => {
    ...
    it("should detect feature-business requirement relationships", () => {
      ...
      const isRelated = (service as any).isFeatureRelatedToBusinessRequirement(
        feature,
        requirement
      );
