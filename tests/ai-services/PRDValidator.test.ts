...
import {
  ...
  type ValidationRule,
} from "../../src/infrastructure/validation/ValidationRuleEngine";
...
describe("PRDValidator", () => {
        ...
        features: [
          {
            ...
            title: "Feature 1",
            description:
              "A detailed description of the feature that is at least 50 characters long",
  ...
  describe("built-in rules", () => {
    ...
    it("checks objectives count (BR-002)", () => {
      ...
      expect(
        validator.validate(onePRD).results.find((r) => r.rule.id === "BR-002")?.result.passed
      ).toBe(false);
      expect(
        validator.validate(twoPRD).results.find((r) => r.rule.id === "BR-002")?.result.passed
      ).toBe(true);
    });
    ...
    it("checks features presence (BR-003)", () => {
      ...
      expect(
        validator.validate(noFeaturesPRD).results.find((r) => r.rule.id === "BR-003")?.result.passed
      ).toBe(false);
      expect(
        validator.validate(withFeaturesPRD).results.find((r) => r.rule.id === "BR-003")?.result
          .passed
      ).toBe(true);
    });
    ...
    it("checks for vague language (CL-003)", () => {
      ...
      const specificPRD = createMinimalPRD({
        objectives: ["Reduce load time by 50%", "Increase throughput to 1000 rps"],
      });

      expect(
        validator.validate(vaguePRD).results.find((r) => r.rule.id === "CL-003")?.result.passed
      ).toBe(false);
      expect(
        validator.validate(specificPRD).results.find((r) => r.rule.id === "CL-003")?.result.passed
      ).toBe(true);
    });
    ...
    it("checks feature descriptions (CL-001)", () => {
        ...
        features: [
          {
            ...
            title: "Feature",
            description:
              "A much longer description that provides more than 50 characters of detail",
            priority: TaskPriority.HIGH,
            acceptanceCriteria: [],
            userStories: [],
            estimatedComplexity: 5,
      ...
      expect(
        validator.validate(shortDescPRD).results.find((r) => r.rule.id === "CL-001")?.result.passed
      ).toBe(false);
      expect(
        validator.validate(longDescPRD).results.find((r) => r.rule.id === "CL-001")?.result.passed
      ).toBe(true);
    });
  });
