...
describe("IterationService", () => {
  ...
  beforeEach(() => {
    ...
    } as unknown as Mocked<ProjectTemplateService>;
  ...
  describe("getIterationConfiguration", () => {
    it("should return iteration config from project fields", async () => {
      mockTemplateService.listProjectFields.mockResolvedValueOnce([
        nonIterationField,
        iterationField,
      ]);
    ...
    it("should throw when no iteration field found", async () => {
      ...
      await expect(service.getIterationConfiguration({ projectId })).rejects.toThrow(DomainError);
    });
    ...
    it("should throw when iteration field has no config", async () => {
      ...
      await expect(service.getIterationConfiguration({ projectId })).rejects.toThrow();
    });
  ...
  describe("getCurrentIteration", () => {
    ...
    it("should return null when no iteration covers today", async () => {
      const gapField: CustomField = {
        ...
        config: {
          iterationDuration: 7,
          iterations: [{ id: "iter-past", title: "Old", startDate: pastStartStr, duration: 7 }],
        },
      };
      ...
      const items: ProjectItem[] = [
        {
          ...
          projectId,
          fieldValues: { PVTF_iter: "iter-current" },
        ...
        },
        {
          ...
          projectId,
          fieldValues: { PVTF_iter: "iter-past" },
          createdAt: "2024-01-01T00:00:00Z",
          updatedAt: "2024-01-02T00:00:00Z",
        },
      ];
