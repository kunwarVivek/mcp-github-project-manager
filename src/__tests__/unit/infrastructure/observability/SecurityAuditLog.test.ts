...
describe('SecurityAuditLog', () => {
  ...
  describe('getRecentEvents', () => {
    it('returns only the latest N events, oldest first among them', () => {
      const log = new SecurityAuditLog(logger);
      for (let i = 0; i < 5; i++) {
        log.record({ type: 'input_sanitization_triggered', source: `s${i}`, details: {}, severity: 'low' });
      }
  ...
  describe('getEventsByType', () => {
    ...
    it('respects the limit, keeping the most recent matches', () => {
      const log = new SecurityAuditLog(logger);
      for (let i = 0; i < 5; i++) {
        log.record({ type: 'rate_limit_exceeded', source: `ip-${i}`, details: {}, severity: 'medium' });
      }
  ...
  describe('severity-based logging', () => {
    it('logs high-severity events at error level', () => {
      const log = new SecurityAuditLog(logger);
      log.record({ type: 'webhook_signature_invalid', source: 'a', details: { x: 1 }, severity: 'high' });

      expect(logger.error).toHaveBeenCalledWith('[SECURITY] webhook_signature_invalid from a', { x: 1 });
