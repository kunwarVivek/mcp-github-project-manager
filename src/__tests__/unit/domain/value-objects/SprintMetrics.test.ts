...
describe("SprintMetrics Value Object", () => {
  ...
  describe("creation", () => {
    ...
    it("should throw error if totalIssues is negative", () => {
      expect(() => SprintMetrics.create({ ...baseConfig, totalIssues: -1 })).toThrow(
        "totalIssues cannot be negative"
      );
    });

    it("should throw error if completedIssues is negative", () => {
      expect(() => SprintMetrics.create({ ...baseConfig, completedIssues: -1 })).toThrow(
        "completedIssues cannot be negative"
      );
    });

    it("should throw error if completedIssues exceeds totalIssues", () => {
      expect(() => SprintMetrics.create({ ...baseConfig, completedIssues: 15 })).toThrow(
        "completedIssues cannot exceed totalIssues"
      );
    });
  });
