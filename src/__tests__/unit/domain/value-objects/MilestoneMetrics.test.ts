...
describe("MilestoneMetrics Value Object", () => {
  ...
  describe("creation", () => {
    ...
    it("should throw error if totalIssues is negative", () => {
      expect(() => MilestoneMetrics.create({ ...baseConfig, totalIssues: -1 })).toThrow(
        "totalIssues cannot be negative"
      );
    });

    it("should throw error if closedIssues is negative", () => {
      expect(() => MilestoneMetrics.create({ ...baseConfig, closedIssues: -1 })).toThrow(
        "closedIssues cannot be negative"
      );
    });

    it("should throw error if closedIssues exceeds totalIssues", () => {
      expect(() => MilestoneMetrics.create({ ...baseConfig, closedIssues: 25 })).toThrow(
        "closedIssues cannot exceed totalIssues"
      );
    });
  });
