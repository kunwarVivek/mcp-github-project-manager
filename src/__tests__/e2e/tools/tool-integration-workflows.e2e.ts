...
describe("Tool Integration Workflows E2E", () => {
  ...
  describe("Complete Project Setup Workflow", () => {
    it("should execute complete project creation workflow", async () => {
      ...
      const prdResponse = await utils.callTool("ai_generate", {
        action: "generate_prd",
        projectIdea:
          "A comprehensive project management dashboard with real-time analytics and team collaboration",
  ...
  describe("AI-Enhanced Project Management Workflow", () => {
    ...
    it("should add new feature and generate tasks", async () => {
      ...
      const featureResponse = await utils.callTool("ai_generate", {
        ...
        description:
          "Add comprehensive analytics with custom reports, data visualization, and export capabilities",
        targetProject: workflowProjectId || "workflow-test",
        businessJustification:
          "Provides valuable insights for decision making and improves user engagement",
        ...
        features: [
          {
            ...
            priority: "high",
            userStories: [
              "As a user, I want to view analytics so that I can make informed decisions",
            ],
            acceptanceCriteria: [
  ...
  describe("Task Management and Optimization Workflow", () => {
    ...
    it("should analyze task complexity for planning", async () => {
      ...
      const complexityResponse = await utils.callTool("ai_generate", {
        ...
        taskDescription:
          "Build a scalable real-time analytics engine that can process large volumes of data and provide instant insights through WebSocket connections",
    ...
    it("should expand complex task into subtasks", async () => {
      ...
      const expandResponse = await utils.callTool("ai_generate", {
        ...
        taskDescription:
          "Create a complete user management system with authentication, authorization, profile management, and admin controls",
        currentComplexity: 8,
        maxSubtasks: 8,
        maxDepth: 2,
        targetComplexity: 3,
  ...
  describe("End-to-End Workflow Validation", () => {
    ...
    it("should demonstrate tool interoperability", async () => {
      ...
      const coreTools = tools.filter((t: any) => t.name.startsWith("manage_"));

      const aiTools = tools.filter((t: any) => t.name.startsWith("ai_"));
