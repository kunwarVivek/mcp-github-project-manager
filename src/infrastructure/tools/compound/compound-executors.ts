...
import { executeGenerateRoadmap, executeGenerateRoadmapVisualization } from "../roadmap-ai-tools";
 ...
 */
function pms(svc: ProjectManagementService, method: string, ...args: unknown[]): unknown {
...
function getEnrichmentService(): IssueEnrichmentService {
  if (!_enrichmentService) {
    _enrichmentService = new IssueEnrichmentService(AIServiceFactory.getInstance(), getPMS());
  }
...
export async function executeManageProject(args: ManageProjectArgs): Promise<unknown> {
  ...
  switch (action) {
    ...
    case "create":
      return toPlain(await pms(svc, "createProject", rest));
    case "update":
      return toPlain(await pms(svc, "updateProject", rest));
    case "delete":
      return toPlain(await pms(svc, "deleteProject", rest));
    case "get_readme":
      return toPlain(await pms(svc, "getProjectReadme", rest));
    case "update_readme":
      return toPlain(await pms(svc, "updateProjectReadme", rest));
    case "create_field":
      return toPlain(await pms(svc, "createProjectField", rest));
    case "list_fields":
      return toPlain(await pms(svc, "listProjectFields", rest));
    case "update_field":
      return toPlain(await pms(svc, "updateProjectField", rest));
    case "create_view":
      return toPlain(await pms(svc, "createProjectView", rest));
    case "list_views":
      return toPlain(await pms(svc, "listProjectViews", rest));
    case "update_view":
      return toPlain(await pms(svc, "updateProjectView", rest));
    case "delete_view":
      return toPlain(await pms(svc, "deleteProjectView", rest));
    case "add_item":
      return toPlain(await pms(svc, "addProjectItem", rest));
    case "remove_item":
      return toPlain(await pms(svc, "removeProjectItem", rest));
    case "list_items":
      return toPlain(await pms(svc, "listProjectItems", rest));
    case "archive_item":
      return toPlain(await pms(svc, "archiveProjectItem", rest));
    case "unarchive_item":
      return toPlain(await pms(svc, "unarchiveProjectItem", rest));
    case "set_field_value":
      return toPlain(await pms(svc, "setFieldValue", rest));
    case "get_field_value":
      return toPlain(await pms(svc, "getFieldValue", rest));
    case "clear_field_value":
      return toPlain(await pms(svc, "clearFieldValue", rest));

    // ── PMS destructured ─────────────────────────────────────────
    case "list":
      return toPlain(await pms(svc, "listProjects", rest.status, rest.limit));
    case "get":
      return toPlain(await pms(svc, "getProject", rest.projectId));

    // ── Standalone executors — lifecycle ──────────────────────────
    case "close":
      return route(executeCloseProject, rest);
    case "reopen":
      return route(executeReopenProject, rest);

    // ── Standalone executors — templates ──────────────────────────
    case "mark_as_template":
      return route(executeMarkProjectAsTemplate, rest);
    case "unmark_as_template":
      return route(executeUnmarkProjectAsTemplate, rest);
    case "copy_from_template":
      return route(executeCopyProjectFromTemplate, rest);
    case "list_templates":
      return route(executeListOrganizationTemplates, rest);

    // ── Standalone executors — linking ────────────────────────────
    case "link_to_repo":
      return route(executeLinkProjectToRepository, rest);
    case "unlink_from_repo":
      return route(executeUnlinkProjectFromRepository, rest);
    case "link_to_team":
      return route(executeLinkProjectToTeam, rest);
    case "unlink_from_team":
      return route(executeUnlinkProjectFromTeam, rest);
    case "list_linked_repos":
      return route(executeListLinkedRepositories, rest);
    case "list_linked_teams":
      return route(executeListLinkedTeams, rest);

    // ── Standalone executors — advanced ──────────────────────────
    case "update_item_position":
      return route(executeUpdateItemPosition, rest);
    case "filter_items":
      return route(executeFilterProjectItems, rest);
    ...
    default:
      unknownAction("manage_project", action);
  }
}
...
export async function executeManageIssues(args: ManageIssuesArgs): Promise<unknown> {
  ...
  switch (action) {
    ...
    case "create":
      return toPlain(await pms(svc, "createIssue", rest));
    case "list":
      return toPlain(await pms(svc, "listIssues", rest));
    case "create_comment":
      return toPlain(await pms(svc, "createIssueComment", rest));
    case "update_comment":
      return toPlain(await pms(svc, "updateIssueComment", rest));
    case "delete_comment":
      return toPlain(await pms(svc, "deleteIssueComment", rest));
    case "list_comments":
      return toPlain(await pms(svc, "listIssueComments", rest));
    case "create_draft":
      return toPlain(await pms(svc, "createDraftIssue", rest));
    case "update_draft":
      return toPlain(await pms(svc, "updateDraftIssue", rest));
    case "delete_draft":
      return toPlain(await pms(svc, "deleteDraftIssue", rest));

    // ── PMS destructured ─────────────────────────────────────────
    case "get":
      return toPlain(await pms(svc, "getIssue", rest.issueId));
    case "update":
      return toPlain(
        await pms(svc, "updateIssue", rest.issueId, {
          title: rest.title,
    ...
    case "convert_draft":
      return route(executeConvertDraftIssue, rest);
    case "search_advanced":
      return route(executeSearchIssuesAdvanced, rest);
    case "add_sub_issue":
      return route(executeAddSubIssue, rest);
    case "list_sub_issues":
      return route(executeListSubIssues, rest);
    case "get_parent_issue":
      return route(executeGetParentIssue, rest);
    case "reprioritize_sub_issue":
      return route(executeReprioritizeSubIssue, rest);
    case "remove_sub_issue":
      return route(executeRemoveSubIssue, rest);

    default:
      unknownAction("manage_issues", action);
  }
}
...
export async function executeManagePrs(args: ManagePrsArgs): Promise<unknown> {
  ...
  switch (action) {
    case "create":
      return toPlain(await pms(svc, "createPullRequest", rest));
    case "get":
      return toPlain(await pms(svc, "getPullRequest", rest));
    case "list":
      return toPlain(await pms(svc, "listPullRequests", rest));
    case "update":
      return toPlain(await pms(svc, "updatePullRequest", rest));
    case "merge":
      return toPlain(await pms(svc, "mergePullRequest", rest));
    case "list_reviews":
      return toPlain(await pms(svc, "listPullRequestReviews", rest));
    case "create_review":
      return toPlain(await pms(svc, "createPullRequestReview", rest));
    default:
      unknownAction("manage_prs", action);
  }
}
...
export async function executeManageMilestones(args: ManageMilestonesArgs): Promise<unknown> {
  ...
  switch (action) {
    // ── PMS passthrough — serialize entity results ────────────────
    case "create":
      return toPlain(await pms(svc, "createMilestone", rest));
    case "update":
      return toPlain(await pms(svc, "updateMilestone", rest));
    case "delete":
      return toPlain(await pms(svc, "deleteMilestone", rest));
    ...
    case "get_upcoming":
      return toPlain(
        await pms(svc, "getUpcomingMilestones", rest.daysAhead, rest.limit, rest.includeIssues)
      );

    default:
      unknownAction("manage_milestones", action);
  }
}
...
export async function executeManageSprints(args: ManageSprintsArgs): Promise<unknown> {
  ...
  switch (action) {
    ...
    case "create": {
      if (!rest.projectId) {
        throw new Error(
          "projectId is required for sprint create — sprints are iteration fields on a specific GitHub project"
        );
      }
      return toPlain(await pms(svc, "createSprint", rest));
    }
    case "update":
      return toPlain(await pms(svc, "updateSprint", rest));
    case "add_issues":
      return toPlain(await pms(svc, "addIssuesToSprint", rest));
    case "remove_issues":
      return toPlain(await pms(svc, "removeIssuesFromSprint", rest));
    case "plan":
      return toPlain(await pms(svc, "planSprint", rest));

    // ── PMS destructured ─────────────────────────────────────────
    case "list":
      return toPlain(await pms(svc, "listSprints", rest.status));
    case "get_current":
      return toPlain(await pms(svc, "getCurrentSprint", rest.includeIssues));
    case "get_metrics":
      return toPlain(await pms(svc, "getSprintMetrics", rest.sprintId, rest.includeIssues));

    default:
      unknownAction("manage_sprints", action);
  }
}
...
export async function executeManageLabels(args: ManageLabelsArgs): Promise<unknown> {
  ...
  switch (action) {
    case "create":
      return pms(svc, "createLabel", rest);
    case "list":
      return pms(svc, "listLabels", rest);
    default:
      unknownAction("manage_labels", action);
  }
}
...
export async function executeManageAutomation(args: ManageAutomationArgs): Promise<unknown> {
  ...
  switch (action) {
    case "create_rule":
      return pms(svc, "createAutomationRule", rest);
    case "update_rule":
      return pms(svc, "updateAutomationRule", rest);
    case "delete_rule":
      return pms(svc, "deleteAutomationRule", rest);
    case "get_rule":
      return pms(svc, "getAutomationRule", rest);
    case "list_rules":
      return pms(svc, "listAutomationRules", rest);
    case "enable_rule":
      return pms(svc, "enableAutomationRule", rest);
    case "disable_rule":
      return pms(svc, "disableAutomationRule", rest);
    default:
      unknownAction("manage_automation", action);
  }
}
...
export async function executeManageIterations(args: ManageIterationsArgs): Promise<unknown> {
  ...
  switch (action) {
    case "get_config":
      return pms(svc, "getIterationConfiguration", rest);
    case "get_current":
      return pms(svc, "getCurrentIteration", rest);
    case "get_items":
      return pms(svc, "getIterationItems", rest);
    case "get_by_date":
      return pms(svc, "getIterationByDate", rest);
    case "assign_items":
      return pms(svc, "assignItemsToIteration", rest);
    default:
      unknownAction("manage_iterations", action);
  }
}
...
export async function executeManageEvents(args: ManageEventsArgs): Promise<unknown> {
  ...
  switch (action) {
    ...
    default:
      unknownAction("manage_events", action);
  }
}
...
export async function executeManageStatusUpdates(args: ManageStatusUpdatesArgs): Promise<unknown> {
  ...
  switch (action) {
    case "create":
      return route(executeCreateStatusUpdate, rest);
    case "list":
      return route(executeListStatusUpdates, rest);
    case "get":
      return route(executeGetStatusUpdate, rest);
    default:
      unknownAction("manage_status_updates", action);
  }
}
...
export async function executeAiGenerate(args: AiGenerateArgs): Promise<unknown> {
  ...
  switch (action) {
    case "generate_prd":
      return route(executeGeneratePRD, rest);
    case "enhance_prd":
      return route(executeEnhancePRD, rest);
    case "parse_prd":
      return route(executeParsePRD, rest);
    case "add_feature":
      return route(executeAddFeature, rest);
    case "get_next_task":
      return route(executeGetNextTask, rest);
    case "analyze_complexity":
      return route(executeAnalyzeTaskComplexity, rest);
    case "expand_task":
      return route(executeExpandTask, rest);
    case "create_traceability_matrix":
      return route(executeCreateTraceabilityMatrix, rest);
    case "materialize_tasks":
      return executeMaterializeTasks(rest);
    default:
      unknownAction("ai_generate", action);
  }
}
...
async function executeMaterializeTasks(args: {
  ...
  if (!projectId) {
    return {
      created: 0,
      message: "projectId is required — agents can only claim issues inside a project",
    };
  }
  ...
  for (let i = 0; i < tasks.length; i++) {
    ...
    graph.addTask({
      ...
      dependencies: (task.dependencies || []).map((d) => ({
        id: d.id,
        type: (d.type || "blocks") as "blocks" | "depends_on" | "related_to",
        description: d.description,
      })),
      acceptanceCriteria: (task.acceptanceCriteria || []).map((ac) => ({
        id: ac.id || `ac-${Math.random().toString(36).slice(2, 6)}`,
        description: ac.description,
        completed: ac.completed ?? false,
      })),
      tags: task.tags || [],
      aiGenerated: true,
      subtasks: [],
  ...
  for (let phaseIdx = 0; phaseIdx < phases.length; phaseIdx++) {
    ...
    for (const taskId of phaseTaskIds) {
      ...
      try {
        ...
        const issueBody = [
          ...
          deps.length > 0
            ? [``, `## Dependencies (must complete first)`, ``, ...deps].join("\n")
            : "",
          relevantContext
            ? [
                ``,
        ...
        try {
          ...
          errors.push({
            title: task.title,
            error: `Issue #${issue.number} created but project-add failed: ${addErr instanceof Error ? addErr.message : String(addErr)}`,
          });
        }
    ...
    const phaseIssueIds = createdIssues.filter((i) => i.phase === phaseIdx).map((i) => i.id);
    if (sprintId && phaseIssueIds.length > 0) {
      try {
        ...
        errors.push({
          title: `Sprint assignment: ${phaseName}`,
          error: sprintErr instanceof Error ? sprintErr.message : String(sprintErr),
        });
      }
    }
  }
...
export async function executeAiAnalyze(args: AiAnalyzeArgs): Promise<unknown> {
  ...
  switch (action) {
    // ── Standalone executors ─────────────────────────────────────
    case "suggest_labels":
      return route(executeSuggestLabels, rest);
    case "detect_duplicates":
      return route(executeDetectDuplicates, rest);
    case "find_related":
      return route(executeFindRelatedIssues, rest);
    ...
    case "enrich_bulk": {
      ...
      const issueIds = rest.issueIds ?? items.map((item) => item.id);
    ...
    default:
      unknownAction("ai_analyze", action);
  }
}
...
export async function executeAiPlan(args: AiPlanArgs): Promise<unknown> {
  ...
  switch (action) {
    case "calculate_capacity":
      return route(executeCalculateSprintCapacity, rest);
    case "prioritize_backlog":
      return route(executePrioritizeBacklog, rest);
    case "assess_risk":
      return route(executeAssessSprintRisk, rest);
    case "suggest_composition":
      return route(executeSuggestSprintComposition, rest);
    case "generate_roadmap":
      return route(executeGenerateRoadmap, rest);
    case "generate_visualization":
      return route(executeGenerateRoadmapVisualization, rest);
    default:
      unknownAction("ai_plan", action);
  }
}
...
export async function executeAgentWork(args: AgentWorkArgs): Promise<unknown> {
  ...
  switch (action) {
    case "register":
      return route(executeRegisterAgent, rest);
    case "checkout_task":
      return route(executeCheckoutTask, rest);
    case "release_task":
      return route(executeReleaseTask, rest);
    case "complete_task":
      return route(executeCompleteTask, rest);
    case "heartbeat":
      return route(executeAgentHeartbeat, rest);
    case "check_work_status":
      return route(executeCheckWorkStatus, rest);
    case "get_task_context":
      return route(executeGetTaskContext, rest);
    case "submit_for_review":
      return route(executeSubmitForReview, rest);
    case "approve_task":
      return route(executeApproveTask, rest);
    case "reject_task":
      return route(executeRejectTask, rest);
    case "validate_work_product": {
      ...
      if (Number.isNaN(issueNumber))
        return { valid: false, findings: ["Invalid taskId — expected issue number"] };
      ...
      if (products.length === 0) {
        return {
          valid: false,
          findings: ["No work product submitted for this task"],
      ...
      if (!latest.testResults) findings.push("No test results attached");
        ...
        if (latest.testResults.failed > 0)
          findings.push(`${latest.testResults.failed} test(s) failing`);
      }
      if (!latest.summary || latest.summary.length < 10)
        findings.push("Work product summary is too brief");

      const valid = findings.length === 0;
      const recommendation = valid
        ? "approve"
        : (latest.testResults?.failed ?? 0) > 0
          ? "reject"
          : "needs_work";
    ...
    case "get_handoff_context": {
      ...
      if (parentMatch) {
        ...
        const latestParentWP =
          parentProducts.length > 0 ? parentProducts[parentProducts.length - 1] : null;
        ...
        const rejectionComment = comments
          .reverse()
          .find((c) => c.body?.includes("## Agent Work Rejected"));
    ...
    default:
      unknownAction("agent_work", action);
  }
}
...
export async function executeAgentManage(args: AgentManageArgs): Promise<unknown> {
  ...
  switch (action) {
    case "list":
      return route(executeListAgents, rest);
    case "deregister":
      return route(executeDeregisterAgent, rest);
    case "get_activity":
      return route(executeGetAgentActivity, rest);
    case "submit_work_product":
      return route(executeSubmitWorkProduct, rest);
    case "get_budget":
      return route(executeGetBudgetStatus, rest);
    case "set_budget":
      return route(executeSetAgentBudget, rest);
    case "record_usage":
      return route(executeRecordUsage, rest);
    case "get_metrics":
      return route(executeGetAgentMetrics, rest);
    case "reclaim_stale":
      return route(executeReclaimStaleTasks, rest);
    case "setup_fields": {
      const factory = createGitHubFactory();
      const setup = new ProjectFieldSetup(factory);
      return setup.ensureFields(rest.projectId as string);
        ...
        agents.map(async (agent) => {
          ...
          return {
            ...
            currentTask: agent.currentTaskId
              ? { id: agent.currentTaskId, title: agent.currentTaskTitle }
              : null,
    ...
    case "smart_assign": {
      ...
      if (eligibleAgents.length === 0)
        return {
          success: false,
          message: "No eligible idle agents (check agentIds, roleFilter, or agent freshness)",
          assignments: [],
        };
      ...
      const projectItemsResp = await factory.graphql<{
        ...
        `query($id: ID!) { node(id: $id) { ... on ProjectV2 { items(first: 100) { nodes { content { ... on Issue { number title state labels(first: 10) { nodes { name } } } } } } } } }`,
        { id: projectId }
      );
      ...
      for (const agent of eligibleAgents) {
        ...
        for (const issue of openIssues) {
          ...
          if (agent.role === "qa" && (labels.includes("testing") || labels.includes("qa")))
            score += 3;
        ...
        if (bestIssue && bestScore > 0) {
          // Attempt assignment
          try {
            const result = await checkoutService.checkoutTask(agent.id, {
              projectId,
              issueNumber: bestIssue.number,
            });
            if (result.success) {
              assignments.push({
                agentId: agent.id,
    ...
    case "decompose_task": {
      ...
      const subtaskDefs = rest.subtasks as Array<{
        title: string;
        description: string;
      ...
      for (const sub of subtaskDefs) {
        ...
        try {
          ...
          /* non-fatal — issue is still created and comment-linked below */
        }
    ...
    case "converge_project": {
      ...
      const projectItemsResp = await factory.graphql<{
        ...
        `query($id: ID!) { node(id: $id) { ... on ProjectV2 { items(first: 100) { nodes { content { ... on Issue { number title state body labels(first: 10) { nodes { name } } } } } } } } }`,
        { id: projectId }
      );
      const openIssues = (projectItemsResp.node?.items?.nodes || [])
        .map((n) => n.content)
      ...
      for (const issue of openIssues) {
        ...
        if (findings.length === 0) {
          ...
          await octokit.rest.issues
            ...
            .catch((err) => {
              logger.warn(`converge: failed to comment approval on issue #${issue.number}`, err);
            });
          await octokit.rest.issues
            .update({
              owner: config.owner,
            ...
            .catch((err) => {
              logger.warn(`converge: failed to close issue #${issue.number}`, err);
            });
          ...
          await octokit.rest.issues
            ...
            .catch((err) => {
              logger.warn(`converge: failed to comment rejection on issue #${issue.number}`, err);
            });
    ...
    case "converge_until_done": {
      ...
      if (iteration > maxIterations)
        return { done: true, reason: "Max iterations reached", iteration };

      // Get project issues
      const projectItemsResp = await factory.graphql<{
        node: {
          items: {
            nodes: Array<{ content: { number: number; title: string; state: string } | null }>;
          };
        } | null;
      }>(
        `query($id: ID!) { node(id: $id) { ... on ProjectV2 { items(first: 100) { nodes { content { ... on Issue { number title state } } } } } } }`,
        { id: projectId }
      );

      const allItems = (projectItemsResp.node?.items?.nodes || [])
        .map((n) => n.content)
      ...
      if (withWorkProduct.some((i) => !i.hasFailures)) {
        nextActions.push(
          `Run converge_project to auto-approve ${withWorkProduct.filter((i) => !i.hasFailures).length} passing issue(s)`
        );
      }
      if (withWorkProduct.some((i) => i.hasFailures)) {
        nextActions.push(
          `Run converge_project to auto-reject ${withWorkProduct.filter((i) => i.hasFailures).length} failing issue(s) and decompose fix subtasks`
        );
      }
      if (withoutWorkProduct.length > 0) {
        nextActions.push(
          `Run smart_assign to assign ${withoutWorkProduct.length} unassigned issue(s) to idle agents`
        );
      }
      if (nextActions.length === 0) {
        nextActions.push(
          "All issues have work products but none are ready — wait for agents to submit updates"
        );
      }
    ...
    default:
      unknownAction("agent_manage", action);
  }
}
...
> = {
  ...
  ai_generation: {
    ...
    description:
      "AI-powered generation — PRDs, tasks, features, traceability, task materialization to GitHub issues",
  },
  ai_analysis: {
    tool: "ai_analyze",
    actions: [
    ...
    description:
      "AI-powered analysis — issue enrichment, triaging, labels, duplicates, relationships",
  },
  ai_planning: {
    tool: "ai_plan",
    actions: [
  ...
  agents: {
    ...
    description:
      "Agent orchestration — task lifecycle (agent_work), administration (agent_manage), PM coordination (assign_task, get_swarm_status, rebalance_workload)",
  },
  system: {
    tool: "system",
...
const AI_DOMAINS: Record<string, true> = {
  ai_generation: true,
  ai_analysis: true,
  ai_planning: true,
};
...
export async function executeDiscoverTools(args: DiscoverToolsArgs): Promise<unknown> {
  ...
  if (query) {
    ...
    const matches = Object.entries(TOOL_CATALOG).filter(
      ([, v]) => v.description.toLowerCase().includes(q) || v.actions.some((a) => a.includes(q))
    );
