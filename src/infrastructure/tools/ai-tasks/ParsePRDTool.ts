...
const parsePRDSchema = z.object({
  ...
  includeSubtasks: z
    .boolean()
    .default(true)
    .describe("Whether to break down complex tasks into subtasks"),
  autoEstimate: z
    .boolean()
    .default(true)
    .describe("Whether to automatically estimate effort and complexity"),
  autoPrioritize: z.boolean().default(true).describe("Whether to automatically prioritize tasks"),
  autoDetectDependencies: z
    .boolean()
    .default(true)
    .describe("Whether to automatically detect task dependencies"),
  targetComplexity: z
    .number()
    .min(1)
    .max(10)
    .optional()
    .describe("Target maximum complexity for individual tasks"),
  teamSkills: z
    .array(z.string())
    .optional()
    .describe("Team skills to consider for task assignment"),
  projectType: z.string().optional().describe("Type of project (web-app, mobile-app, api, etc.)"),
  createLifecycle: z
    .boolean()
    .default(true)
    .describe("Whether to create lifecycle tracking for tasks"),
  createTraceabilityMatrix: z
    .boolean()
    .default(true)
    .describe("Whether to create comprehensive requirements traceability matrix"),
  includeUseCases: z
    .boolean()
    .default(true)
    .describe("Whether to generate use cases from features"),
  projectId: z.string().optional().describe("Project ID for traceability matrix"),
  enhancedGeneration: z
    .boolean()
    .default(true)
    .describe("Whether to use enhanced task generation with context"),
  contextLevel: z
    .enum(["minimal", "standard", "full"])
    .default("standard")
    .describe("Level of contextual information to include"),
  includeBusinessContext: z
    .boolean()
    .default(false)
    .describe("Whether to include AI-generated business context (requires AI)"),
  includeTechnicalContext: z
    .boolean()
    .default(false)
    .describe("Whether to include AI-generated technical context (requires AI)"),
  includeImplementationGuidance: z
    .boolean()
    .default(false)
    .describe("Whether to include AI-generated implementation guidance (requires AI)"),
});
...
async function generateTaskRecommendations(tasks: any[], _args: ParsePRDArgs) {
  ...
  const complexTasks = tasks.filter((task) => task.complexity >= 8).slice(0, 3);
...
function formatPRDParsingResult(
  ...
  sections.push(
    ...
    "",
    "**By Priority:**",
    ...Object.entries(metrics.tasksByPriority).map(
      ([priority, count]) => `- ${priority}: ${count} task${(count as number) > 1 ? "s" : ""}`
    ),
    "",
    "**By Complexity:**",
    ...Object.entries(metrics.tasksByComplexity).map(
      ([level, count]) => `- ${level}: ${count} task${(count as number) > 1 ? "s" : ""}`
    ),
    ""
  );
  ...
  if (features.length > 0) {
    sections.push(
      ...
      "",
      "**By Priority:**",
      ...Object.entries(metrics.featuresByPriority).map(
        ([priority, count]) => `- ${priority}: ${count} feature${(count as number) > 1 ? "s" : ""}`
      ),
      "",
      "**Top Features:**",
      ...features
  ...
  if (recommendations.startWithTasks.length > 0) {
    sections.push(
      ...
      ...recommendations.startWithTasks.map(
        (task: any) => `- ${task.title} (${task.priority}, ${task.estimatedHours}h)`
      ),
      ""
    );
  }
  ...
  if (recommendations.complexTasksNeedingBreakdown.length > 0) {
    sections.push(
      ...
      ...recommendations.complexTasksNeedingBreakdown.map(
        (task: any) => `- ${task.title} (complexity: ${task.complexity}/10)`
      ),
      ""
    );
  }
  ...
  if (recommendations.recommendedFirstSprint.length > 0) {
    const sprintEffort = recommendations.recommendedFirstSprint.reduce(
  ...
  // Project insights
  sections.push("## Project Insights", "");
