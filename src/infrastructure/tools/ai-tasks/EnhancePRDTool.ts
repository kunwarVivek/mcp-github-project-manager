...
const enhancePRDSchema = z.object({
  ...
  industryContext: z.string().optional().describe("Industry or domain context for enhancement"),
  addMissingElements: z
    .boolean()
    .default(true)
    .describe("Whether to add commonly missing PRD elements"),
  improveExisting: z.boolean().default(true).describe("Whether to improve existing sections"),
  validateQuality: z
    .boolean()
    .default(true)
...
function formatPRDEnhancement(
  ...
  if (features.length > 0) {
    sections.push("## Feature Analysis", `**Total Features:** ${features.length}`, "");
    ...
    sections.push(
      ...
      ...Object.entries(featuresByPriority).map(
    ...
    if (topFeatures.length > 0) {
      sections.push(
        ...
        ...topFeatures.map(
          (feature: any) => `- ${feature.title} (complexity: ${feature.estimatedComplexity}/10)`
        ),
        ""
      );
    }
  ...
  if (enhancedPRD.technicalRequirements && enhancedPRD.technicalRequirements.length > 0) {
    ...
    sections.push(
      "**By Category:**",
      ...Object.entries(reqsByCategory).map(
        ([category, count]) =>
  ...
  if (enhancedPRD.targetUsers && enhancedPRD.targetUsers.length > 0) {
    sections.push("## User Personas", `**Total Personas:** ${enhancedPRD.targetUsers.length}`, "");

    enhancedPRD.targetUsers.slice(0, 3).forEach((user: any) => {
      sections.push(`**${user.name}** (${user.technicalLevel})`, `- ${user.description}`, "");
    });
  }
...
// Tool definition
export const enhancePRDTool: ToolDefinition<
  EnhancePRDArgs,
  z.infer<typeof PRDEnhanceOutputSchema>
> = {
  name: "enhance_prd",
  title: "Enhance PRD",
  description:
