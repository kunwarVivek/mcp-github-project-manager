...
const generatePRDSchema = z.object({
  ...
  timeline: z
    .string()
    .optional()
    .describe('Expected project timeline (e.g., "3 months", "Q1 2024")'),
  complexity: z
    .enum(["low", "medium", "high"])
    .default("medium")
    .describe("Expected project complexity"),
...
function formatPRDGenerationSummary(prd: any, validation: any): string {
  ...
  if (prd.features && prd.features.length > 0) {
    sections.push("## Key Features", `**Total Features:** ${prd.features.length}`, "");
  ...
  examples: [
    {
      ...
      args: {
        projectIdea:
          "A modern task management application with AI-powered prioritization, team collaboration features, and integration with popular development tools like GitHub and Slack",
