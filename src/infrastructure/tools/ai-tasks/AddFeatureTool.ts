...
const addFeatureSchema = z.object({
  ...
  autoApprove: z
    .boolean()
    .default(false)
    .describe("Whether to auto-approve the feature without manual review"),
  expandToTasks: z
    .boolean()
    .default(true)
    .describe("Whether to immediately expand the feature into tasks"),
  createLifecycle: z
    .boolean()
    .default(true)
    .describe("Whether to create complete task lifecycle management"),
});
  ...
  if (result.expansionResult) {
    ...
    sections.push(
      ...
      ...Object.entries(tasksByPriority).map(
        ([priority, count]) => `- ${priority}: ${count} tasks`
      ),
      ""
    );
    ...
    if (highPriorityTasks.length > 0) {
      sections.push(
        ...
        ...highPriorityTasks.map(
          (task: any) => `- ${task.title} (${task.complexity}/10, ${task.estimatedHours}h)`
        ),
        ""
      );
    }
...
export const addFeatureTool: ToolDefinition<
  AddFeatureArgs,
  z.infer<typeof AddFeatureOutputSchema>
> = {
  name: "add_feature",
  title: "Add Feature",
  description:
  ...
  examples: [
    {
      ...
      args: {
        ...
        description:
          "Implement a comprehensive user authentication system with login, registration, password reset, and multi-factor authentication",
