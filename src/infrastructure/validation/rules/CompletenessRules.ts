...
export const COMPLETENESS_RULES: ValidationRule[] = [
...

  {
    ...
    check: (prd: PRDDocument) => {
      ...
      return {
        passed: hasFeatures,
        message: hasFeatures ? `PRD has ${featureCount} features` : "PRD has no features defined",
        location: "features",
...

  {
    ...
    check: (prd: PRDDocument) => {
      ...
      return {
        passed: hasTimeline,
        message: hasTimeline ? "PRD has timeline specified" : "PRD has no timeline or schedule",
        location: "timeline",
