...
export class ContextualReferenceGenerator {
   ...
   */
  private extractPRDSections(_task: AITask, prd: PRDDocument | null): PRDSectionReference[] {
    if (!prd) {
      return [
        {
          section: "Overview",
   ...
   */
  private identifyRelatedFeatures(task: AITask, features: FeatureRequirement[]): RelatedFeature[] {
    ...
    for (const feature of features) {
      ...
      if (taskTitleLower.includes(featureTitleLower) || taskDescLower.includes(featureTitleLower)) {
   ...
   */
  private extractTechnicalSpecs(task: AITask, _prd: PRDDocument | null): TechnicalSpecReference[] {
    ...
    if (
      taskText.includes("architecture") ||
      taskText.includes("service") ||
      taskText.includes("system")
    ) {
      specs.push({
