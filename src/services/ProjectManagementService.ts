...
export class ProjectManagementService {
  ...
  async listIssueComments(data: {
    ...
    Array<{ id: number; body: string; user: string; createdAt: string; updatedAt: string }>
  > {
  ...
  async listPullRequestReviews(data: {
    pullNumber: number;
  }): Promise<Array<{ id: number; user: string; state: string; body: string }>> {
  ...
  async listLabels(
    data: { limit?: number } = {}
  ): Promise<Array<{ id: number; name: string; color: string; description: string }>> {
  ...
  ): Promise<Omit<AutomationRuleDTO, "projectId">> {
  ...
  async listAutomationRules(data: {
    projectId: string;
  }): Promise<{ rules: AutomationRuleSummary[] }> {
    return safeCall(async () => ({
      rules: await this.automationService.listRuleSummaries(data.projectId),
    }));
  }
  async enableAutomationRule(data: {
    ruleId: string;
  ...
  async disableAutomationRule(data: {
    ruleId: string;
  }): Promise<{ id: string; name: string; enabled: boolean }> {
    return safeCall(async () => {
  ...
  get iterations(): IterationService {
