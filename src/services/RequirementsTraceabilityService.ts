...
export class RequirementsTraceabilityService {
  ...
  enhanceTasksWithTraceability(
    ...
    return tasks.map((task) => {
      ...
      const relatedUseCases = useCases.filter((uc) => this.isTaskRelatedToUseCase(task, uc));
      ...
      const impactAnalysis = {
        ...
        riskLevel: this.assessTaskRiskLevel(task, relatedUseCases.length, relatedFeatures.length) as
          | "low"
          | "medium"
          | "high",
      };
  ...
  private calculateCoverage(
    ...
    const tasksWithTraceability = tasks.filter(
      (task) =>
  ...
  private isFeatureRelatedToBusinessRequirement(
    feature: FeatureRequirement,
