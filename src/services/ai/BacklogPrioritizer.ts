...
export class BacklogPrioritizer {
  ...
  async prioritize(params: {
    ...
    const confidence = this.calculateConfidence(params.backlogItems, aiScoring, graphAnalysis);
  ...
  private async getAIBusinessValueScoring(
    ...
    try {
      const result = await generateObject({
        ...
        prompt: formatPrioritizationPrompt(
          items.map((item) => ({ ...item, title: InputSanitizer.sanitizeText(item.title) })),
          goals?.map((g) => InputSanitizer.sanitizeText(g)) ?? []
        ),
   ...
   */
  private calculateRiskScore(item: BacklogItem, riskTolerance: "low" | "medium" | "high"): number {
  ...
  private formatReasoning(
    ...
    return (
      `Score ${item.score}: BV=${factors.businessValue.toFixed(2)}, ` +
  ...
  private calculateConfidence(
    ...
    const usedAI = !aiScoring.tradeoffs.some(
      (t) => t.includes("Fallback") || t.includes("AI unavailable") || t.includes("AI error")
    );
