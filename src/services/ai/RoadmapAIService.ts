...
IMPORTANT: Follow phase sequencing rules - foundation first, then core, then advanced features.`;
  ...
  /**
   * Calculate milestone dates from the velocity-scaled structure.
   *
   * Velocity is applied upstream: generateRoadmap computes totalWeeks from
   * velocity (line 147-148), and the structure generator (AI or fallback)
   * distributes that into per-phase durationWeeks. This function therefore
   * uses durationWeeks directly — applying velocity here would double-scale.
   */
