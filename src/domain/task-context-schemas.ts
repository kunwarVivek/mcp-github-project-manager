...
export const BestPracticeSchema = z.object({
  category: z
    .enum(["coding", "architecture", "security", "performance", "testing"])
...
export const ImplementationGuidanceSchema = z.object({
  recommendedApproach: z.string().min(50).describe("Overall implementation strategy and approach"),
  implementationSteps: z.array(z.string()).min(3).describe("Step-by-step implementation guide"),
  technicalConsiderations: z
    .array(z.string())
    .min(1)
    .describe("Important technical points to consider"),
  commonPitfalls: z.array(z.string()).min(1).describe("Common mistakes to avoid"),
  testingStrategy: z.string().min(20).describe("How to test this implementation"),
  recommendedTools: z.array(z.string()).describe("Recommended tools and technologies"),
  codeQualityStandards: z.array(z.string()).describe("Quality checks and standards to maintain"),
  performanceConsiderations: z
    .array(z.string())
...
export const RelatedFeatureSchema = z.object({
  ...
  title: z.string().min(3).describe("Feature title"),
  relationship: z
    .enum(["implements", "extends", "integrates_with", "depends_on", "enables"])
...
export const TechnicalSpecReferenceSchema = z.object({
  type: z
    .enum(["api_spec", "data_model", "architecture_doc", "design_system", "protocol"])
...
export const ExternalReferenceSchema = z.object({
  type: z
    .enum(["documentation", "tutorial", "best_practice", "tool", "library"])
...
export const ContextualReferencesSchema = z.object({
  prdSections: z.array(PRDSectionReferenceSchema).describe("Relevant PRD sections with context"),
  relatedFeatures: z
    .array(RelatedFeatureSchema)
    .describe("Related features and their relationships"),
  technicalSpecs: z
    .array(TechnicalSpecReferenceSchema)
    .describe("Technical specification references"),
  codeExamples: z.array(CodeExampleSchema).describe("Relevant code examples"),
...
export const EnhancedAcceptanceCriterionSchema = z.object({
  ...
  category: z
    .enum(["functional", "technical", "quality", "integration", "performance", "security"])
    .describe("Category of acceptance criterion"),
  description: z.string().min(20).describe("Clear, testable description"),
  verificationMethod: z
    .enum(["unit_test", "integration_test", "manual_test", "code_review", "demo", "automated_test"])
    .describe("How to verify this criterion"),
  verificationDetails: z.string().min(20).describe("Detailed verification instructions"),
  priority: z.enum(["must_have", "should_have", "nice_to_have"]).describe("Priority level"),
  acceptanceThreshold: z.string().min(10).describe("Specific threshold or success condition"),
  testingNotes: z.string().optional().describe("Additional testing notes"),
});
...
export const EnhancedAcceptanceCriteriaSchema = z.object({
  criteria: z
    .array(EnhancedAcceptanceCriterionSchema)
    .min(1)
  ...
  coverageAnalysis: z
    ...
    .optional()
    .describe("Coverage analysis metrics"),
});
...
export const DependencySchema = z.object({
  ...
  dependencyType: z
    .enum(["blocks", "required_by", "relates_to", "implements"])
    .describe("Type of dependency"),
  rationale: z.string().min(20).describe("Why this dependency exists"),
  providedBy: z.string().min(10).describe("What the dependent task provides"),
  integrationGuidance: z.string().min(20).describe("How to integrate with this dependency"),
  interfaces: z.array(z.string()).describe("Integration interfaces or contracts"),
  canRunInParallel: z.boolean().describe("Whether this can be worked on in parallel"),
});
...
export const DependencyContextSchema = z.object({
  dependencies: z.array(DependencySchema).describe("Detailed dependency information"),
  parallelOpportunities: z
    .array(
      z.object({
...
export const TaskExecutionContextSchema = z.object({
  ...
  businessObjective: z.string().min(20).describe("Primary business objective this task supports"),
  userImpact: z.string().min(20).describe("Specific impact on end users"),
  successMetrics: z.array(z.string()).min(1).describe("Measurable success indicators"),

  // Feature Context
  parentFeature: FeatureContextSchema.describe("Parent feature information"),
  ...
  architecturalDecisions: z.array(z.string()).describe("Relevant architectural decisions"),
  integrationPoints: z.array(z.string()).describe("System integration requirements"),
  dataRequirements: z.array(z.string()).describe("Data-related requirements"),

  // PRD Context Summary
  prdContextSummary: PRDContextSummarySchema.describe("Summary of relevant PRD content"),

  // Enhanced Context (Optional - requires AI)
  implementationGuidance: ImplementationGuidanceSchema.optional().describe(
    "AI-generated implementation guidance (FR-3)"
  ),
  contextualReferences: ContextualReferencesSchema.optional().describe(
    "Contextual references to PRD, specs, examples (FR-4)"
  ),
  enhancedAcceptanceCriteria: EnhancedAcceptanceCriteriaSchema.optional().describe(
    "Enhanced acceptance criteria with verification (FR-5)"
  ),
  dependencyContext: DependencyContextSchema.optional().describe(
    "Enhanced dependency context (FR-6)"
  ),
});
...
export const ContextQualityMetricsSchema = z.object({
  ...
  cacheHit: z.boolean().describe("Whether this context was retrieved from cache"),
  aiEnhanced: z.boolean().describe("Whether AI enhancement was used"),
  errors: z.array(z.string()).describe("Any errors encountered during generation"),
  warnings: z.array(z.string()).describe("Any warnings or issues"),
});
...
export const EnhancedTaskWithContextSchema = z.object({
  ...
  // Execution Context
  executionContext: TaskExecutionContextSchema.describe("Complete execution context for the task"),

  // Quality Metrics
  contextQuality: ContextQualityMetricsSchema.optional().describe(
    "Quality metrics for generated context"
  ),
