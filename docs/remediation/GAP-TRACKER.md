...
| G2-07 | EventStore unbounded memory (no LRU eviction) | EventStore.ts | MEDIUM | NOT-A-GAP (verified: FIFO eviction at maxEventsInMemory via splice, `addToMemoryBuffer` L218-220, with correct `rebuildEventIndex` L232-237. CONCERNS claim stale. Minor: rebuild is O(n)/evict — perf-only, not a bug) |
| G2-08 | ResourceCache: `maxSize` option unenforced (no size eviction, no TTL sweep — only lazy expiry on access) | ResourceCache.ts | MEDIUM | DEFERRED — needs data-model change first: entries store `tags` but not `type`/`namespaces`, so correct LRU eviction (cleaning type/tag/namespace indexes) requires storing namespaces per-entry. Documented; do with D-layer cache work to avoid index corruption |
...
| G3-09 | **Duplicate/orphaned automation service**: `ProjectAutomationService` exists (createRule/updateRule/enable/disable/…) but is wired to nothing; PMS has parallel inline automation methods (createAutomationRule/… using automationRepo, ~210 lines). Different signatures (domain-typed vs tool-shaped) + different DI (@inject AutomationRuleRepository vs factory getter). | MEDIUM | OPEN — design decision: reconcile PMS automation to delegate to ProjectAutomationService (reuse, adapt signatures + register its DI deps) vs extract a fresh tool-shaped AutomationService. Needs user steer |
